...
def write_sheet(loc, wb_name, title_list, title_width, kl, gl):
    """
                ...
                for m in range(sheet.ncols):  # for every column in that row
                    if m == 4:  # if its the date
                        try:  # try formatting the date to excel date format
                            datetime = xlrd.xldate_as_datetime(j[m], 0)  # turns the excel date into a datetime object
...
sheet_path = "Test - SRU.xlsx"  # name of full spreadsheet
wb_name = 'Test - SRU Keywords.xlsx'  # new workbook name

# headings
titles = ['Group Name', 'About']
# width of headings
col_width = [43, 150]
