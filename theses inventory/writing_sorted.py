    ...
    for i in range(len(worksheet)):  # going through each worksheet
        ...
        if i == 1:  # if its the remove worksheet
            for j in range(len(newList[1])):  # goes through the removed rows of the newList
                ...
                # 10 is the index for the 'Keyword(s)' column
                myList = newList[1][j][:10] + newList[1][j][-1:]
                for k in range(len(myList)):  # runs through each element of that new list
                    if k == len(myList) - 1 or k == len(myList):  # if its the keywords or removal reason column
            ...
            for j in range(len(newList[i])):  # goes through the rows of the lists
                for k in range(len(newList[i][j])):  # goes through the columns of the rows
                    if k == len(newList) - 1 or k == len(newList):  # if its the keywords or SDG covered column
...
def read_file(file_name_list):
    ...
    for i in range(len(newList)):  # going through newList
        for j in range(line_count[i]):  # going through each row of each file
            final_list[i].append(
                newList[i][int(12 * j):int(12 * (j + 1))])  # adding the rows of each file to the final list
                # 12 here is the number of columns + 1 (including the keywords and SDGs columns)
...
def formatting(file_name_list, worksheet_list, workbook_list, title_list, title_width):
    ...
    for i in range(len(file_name_list)):
        ...
        if i == 1:  # if its the remove spreadsheet
            for j in range(len(title_list) - 1):  # -1 is to exclude the SDG groups
            ...
            worksheet_list[i].set_column(len(title_list) - 1, len(title_list) - 1, 40)
            worksheet_list[i].write(0, len(title_list) - 1, 'Removal Reason', bold)
        else:
...
wb_names = ['Masters - Keep.xlsx', 'Masters - Deleted.xlsx', 'Masters - Saved.xlsx']  # list of spreadsheet names
