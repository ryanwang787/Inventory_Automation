        ...
        if i == 1:  # if its the remove worksheet
            for j in range(len(newList[1])):  # goes through the removed rows of the newList
                ...
                for k in range(len(myList)):  # runs through each element of that new list
                    if k == 9 or k == 10:  # if its the keywords or removal reason column
            ...
            for j in range(len(newList[i])):  # goes through the rows of the lists
                for k in range(len(newList[i][j])):  # goes through the columns of the rows
                    if k == 9 or k == 10:  # if its the keywords or SDG covered column
...
def read_file(file_name_list):
    ...
    for i in range(len(newList)):  # going through newList
        for j in range(line_count[i]):  # going through each row of each file
            final_list[i].append(
                newList[i][int(28 * j):int(28 * (j + 1))])  # adding the rows of each file to the final list
...
wb_names = ['sgs programs - keep.xlsx', 'sgs programs - deleted.xlsx', 'sgs programs - saved.xlsx']  # list of spreadsheet names

titles = ['Program Name', 'Graduate Unit', 'Department Website', 'Calendar URL', 'SGS URL', 'Faculty Affiliation',
          'Campus', 'Degree(s)',
          'Program Description', 'Keyword(s)']  # list of column titles
col_width = [40, 40, 20, 20, 20, 40, 20, 40, 80, 40]  # list of column widths
