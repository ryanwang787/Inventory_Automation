...
def read_write(loc, name, title_list, title_width):
    ...
    row_list = remove_summer(row_list)
...
def remove_summer(list):
    newList = []
    for i in list:
        if 'summer' not in i[1].lower():
            newList.append(i)
    return newList
...
def merge(list, sheet):
    ...
    for i in range(len(list)):
        ...
        if list[i][0] == list[i - 1][0]:  # if the one before it is a duplicate
            ...
            '''if list[i][0] not in list[i - 1][0]:
                # add the SDG to the keywords list
                list[i][6] = list[i][6] + ', ' + list[i - 1][6]
            if list[i][8] not in list[i - 1][8]:
                list[i][8] = list[i][8] + ', ' + list[i - 1][8]
            if list[i][10] not in list[i - 1][10]:
                list[i][10] = list[i][10] + ', ' + list[i - 1][10]
            if list[i][12] not in list[i - 1][12]:
                list[i][12] = list[i][12] + ', ' + list[i - 1][12]'''
...
def keep_last(newList):
    ...
    for i in range(len(newList)):
        if i < len(newList) - 1:
            if newList[i][0] == newList[i + 1][0]:  # if the row after it is the same
...
sheet_path = '2021-08-13 - CM Course Data.xlsx'  # name of SORTED spreadsheet to be highlighted
wb_name = 'remove dups CM courses.xlsx'  # new workbook name
# headings
titles = ['code', 'startSession', 'title', 'description']
# width of headings
col_width = [20, 20, 60, 120]
