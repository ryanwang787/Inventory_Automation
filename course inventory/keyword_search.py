...
def search(list, sheet, worksheet, keyword, SDGs):
    ...
    for i in list:
        ...
        for j in range(len(keyword)):
            ...
            if keyword[j] in str(i[1]).lower() or keyword[j] in str(i[2]).lower():
                write = True
                ...
                for m in range(sheet.ncols):  # for every column in that row
                    if m == 4:  # if its the date
                        try:  # try formatting the date to excel date format
                            datetime = excelrd.xldate_as_datetime(i[m], 0)  # turns the excel date into a datetime object
...
sheet_path = "TEST - Missed courses.xlsx"  # name of full spreadsheet
wb_name = 'TEST Missed Course Keywords.xlsx'  # new workbook name
