...
def display(row):
    ...
    :return: None
    """
    print('Course Title: ' + row[1], '\nCourse Code: ' + row[0], '\nKeywords: ' + row[7], '\nSDG Groups: ' + row[8])  # displays title, keywords, SDGs
...
def readmore(row):
    ...
    :return: None
    """
    print("Course Description: ", row[2])  # displaying abstract
...
def write(file, file_list):
    ...
    for i in range(len(file_list)):
        for j in range(len(file_list[i])):
                file.write(str(file_list[i][j]) + '@%&')  # adding a '@%&' at the end of each element to separate the columns
        file.write('\n')  # adds a new line at the end of each row


sheet_path = 'Missed Course Removed Dups.xlsx'  # spreadsheet we are sorting through
...
for i in range(len(row_list)):
    for j in range(len(row_list[i])):
        if type(row_list[i][j]) is str:
            row_list[i][j] = row_list[i][j].replace('\n', ' ')

list_list = ['keep_list.txt', 'del_list.txt', 'save_list.txt', 'checked_rows.txt']  # list of lists
...
checked_file = open(list_list[3], 'r', encoding='utf-8')  # reading which rows we've already sorted
...
keep_file = open(list_list[0], 'a', encoding='utf-8')
del_file = open(list_list[1], 'a', encoding='utf-8')
saved_file = open(list_list[2], 'a', encoding='utf-8')
checked_file = open(list_list[3], 'w', encoding='utf-8')
